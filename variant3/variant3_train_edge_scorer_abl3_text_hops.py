"""
Train EdgeScorer Ablation 3: Text Embeddings + Hops

Ablation study to verify that graph embeddings provide additional value.
This version uses text embeddings AND hop information (but no graph embeddings).

Key differences from dual:
- No graph embeddings
- Text embeddings + hop information only
- Tests if graph adds value beyond text+hop
"""
...
from variant3.variant3_edge_scorer_abl3_text_hops import EdgeScorerTextHops, count_parameters


class EdgeScorerTextHopsDataset(Dataset):
    """PyTorch dataset for text+hops ablation"""
    ...
    def __getitem__(self, idx):
        ...
        # Convert hop from 1-indexed to 0-indexed
        hop_idx = hop - 1
...
def main():
    print("=" * 80)
    print("Variant 3 Ablation 3: Text Embeddings + Hops (No Graph)")
    print("=" * 80)
    ...
    WANDB_RUN_NAME = "variant3-ablation3-text-hops"
    ...
    if USE_WANDB:
        wandb.init(
            ...
            name=WANDB_RUN_NAME,
            config={
                "architecture": "EdgeScorerTextHops",
                "ablation": "text_hops",
    ...
    train_dataset = EdgeScorerTextHopsDataset(
        train_samples,
        text_embeddings_cache
    )
    val_dataset = EdgeScorerTextHopsDataset(
        val_samples,
        text_embeddings_cache
    )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_metrics['f1'] > best_f1:
            ...
            # Save best model
            checkpoint_path = Config.BASE_DIR / "models" / "variant3_edge_scorer_abl3_text_hops_best.pt"
    ...
    history_path = models_dir / "variant3_training_history_abl3_text_hops.json"
    ...
    csv_path = models_dir / "variant3_training_history_abl3_text_hops.csv"
    ...
    summary_path = models_dir / "variant3_training_summary_abl3_text_hops.json"
    ...
    checkpoint_path = models_dir / "variant3_edge_scorer_abl3_text_hops_best.pt"
    ...
    val_results_path = models_dir / "variant3_validation_results_abl3_text_hops.json"
    ...
    print("\n" + "=" * 80)
    print("Ablation 3 (Text + Hops) complete!")
    print("=" * 80)
